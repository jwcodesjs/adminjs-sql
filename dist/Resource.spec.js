...
async function getResource(table) {
...
describe("Resource", () => {
    ...
    describe(".isAdapterFor", () => {
        it("returns true when resource metadata class is given", () => {
            const resourceMetadata = new ResourceMetadata(dbConfig.config.dialect, dbConfig.knex, dbConfig.config.database, dbConfig.config.schema || dbConfig.config.database, "user", [
                new Property({
                    isEditable: false,
                    isId: true,
                    isEnum: false,
        ...
        it("returns false for any other kind of resources", () => {
            expect(Resource.isAdapterFor({})).toEqual(false);
        });
    });
    ...
    describe("#count", () => {
        it("returns number of records", async () => {
            let count = await database.resource("post").count({});
            expect(count).toBeGreaterThanOrEqual(0);
            const user = await fixtures.createUser();
            const post = await database
                .resource("post")
                .create(buildPost({ id: user.id }));
            const filter = new Filter({ author_id: post.author_id }, database.resource("post"));
    ...
    describe("#find", () => {
        ...
        it("finds by enum column", async () => {
            ...
            expect(records).toSatisfyAll((record) => record.get("status") === post.status);
