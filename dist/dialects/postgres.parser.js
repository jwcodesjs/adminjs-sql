...
export class PostgresParser extends BaseDatabaseParser {
    ...
    async getTables(schemaName, parseOptions) {
        ...
        return result
            ...
            .filter((table) => !parseOptions.ignoredTables.includes(table));
    }
    async getResources(tables, schemaName) {
        const resources = await Promise.all(tables.map(async (tableName) => {
            try {
                const resourceMetadata = new ResourceMetadata(this.dialect, this.knex, this.connectionOptions.database, schemaName, tableName, await this.getProperties(tableName));
    ...
    async getProperties(table) {
        ...
        const query = this.knex
            ...
            .select("col.column_default", "col.column_name", "col.data_type", "col.is_nullable", "col.is_updatable", "col.ordinal_position", "col.udt_name", "col.udt_schema", "tco.constraint_type as key_type")
    ...
    `);
        ...
        function getColumnInfo(column, enums = {}) {
            const isUserDefined = column.data_type === "USER-DEFINED";
            const availableValues = isUserDefined && enums[`${column.udt_schema}.${column.udt_name}`]
            ...
            return {
                ...
                type: column.referenced_table
                    ? "reference"
                    : getColumnType(column.data_type),
            };
        }
    }
    async getEnums() {
        ...
        const query = (await this.knex
            ...
            .groupBy("namespace.nspname", "type.typname", "enum.enumlabel"));
        const enums = {};
        const createKey = (entry) => `${entry.schema}.${entry.name}`;
