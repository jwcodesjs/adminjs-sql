export const isMySqlDialect = (dialect) => ["mysql", "mariadb"].includes(dialect);
const getMigration = (schema, dialect) => ({
    async up(knex) {
        let upQuery = knex.schema;
        if (dialect === "postgresql") {
            upQuery = upQuery.raw("CREATE TYPE public.status_enum AS ENUM ('active-status-enum', 'inactive-status-enum');");
        }
        return upQuery
            .withSchema(schema)
...
export const getMigrationSource = (schema, dialect) => {
